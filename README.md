...
```
sudo apt-get install python-pygraphviz python-qscintilla2 python-qt4
```
...
For complex software or actions a clean structure if often required. A finite state machine provides such a structure. There are two subgroups of states: normal an end. If you create a state machine two states will be initially generated (start_state, finished_state). The 'start_state' is a 'normal' state and will be the initial state of the finite state machine. The 'finished_state' is as the name indicates an 'end' state, without any state handler. Normal state must always have a state handler which returns the next state and maybe some data to feed next state. The return-statement should be in folloing pattern: ```python return ('new_state', cargo)```. A regex parser will search for this pattern to display a complete graph of your state machine in editor window.
...
- Start client node with '3 5' as args.
